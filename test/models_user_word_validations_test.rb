...
class ModelsUserWordValidationsTest < Minitest::Test
  ...
  def test_requires_foreign_keys_and_show_count
    ...
    assert_includes user_word.errors[:user_id], "can't be blank"
    assert_includes user_word.errors[:word_id], "can't be blank"
    assert_includes user_word.errors[:leitner_box_id], "can't be blank"
    assert_includes user_word.errors[:show_count], "can't be blank"
  end
  ...
  def test_learned_inclusion
    ...
    refute user_word.valid?
    assert_includes user_word.errors[:learned], 'is not included in the list'
  end
end
